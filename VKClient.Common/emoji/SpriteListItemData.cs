...
namespace VKClient.Common.Emoji
{
  public class SpriteListItemData : ViewModelBase, ISupportBackspaceListItem, IHandle<NewStoreItemsCountChangedEvent>, IHandle
  {
    ...
    public string TabThumbSticker
    {
      get
      {
        if (this.StickerProduct == null)
          return "";
        return string.Format("{0}thumb_102.png", this.StickerProduct.base_url);
      }
    }
    ...
    public Visibility NoStickersVisibility
    {
      get
      {
        ...
        if (this.IsStickersPack || this.IsRecentStickers)
        {
          ...
          if ((stickers != null ? stickers.sticker_ids :  null) != null)
          {
    ...
    {
      this.StickerStockItemHeader = new StockItemHeader(new StockItem(), false, 0, false);
      EventAggregator.Current.Subscribe(this);
    }

    public void ProcessSystemTab(long userOrChatId, bool isChat)
    {
      if (this.IsStore)
      {
        CurrentStickersPurchaseFunnelSource.Source = StickersPurchaseFunnelSource.keyboard;
        EventAggregator.Current.Publish(new StickersPurchaseFunnelEvent(StickersPurchaseFunnelAction.store));
        Navigator.Current.NavigateToStickersStore(userOrChatId, isChat);
      }
      else
      {
        if (!this.IsSettings)
    ...
    private void UpdateStoreCounter()
    {
        ...
        {
            base.NotifyPropertyChanged<int>(() => this.Counter);
            base.NotifyPropertyChanged<Visibility>(() => this.CounterVisibility);
        });
    }
