...
namespace VKClient.Common.VideoCatalog
{
  public class CatalogItemViewModel : ViewModelBase
  {
    ...
    public string Subtitle1
    {
      get
      {
        ...
        if (this.IsVideo)
        {
          ...
          {
              Group group = (Group)Enumerable.FirstOrDefault<Group>(this._knownGroups, (Func<Group, bool>)(g => g.id == -this._item.owner_id));
            if (group != null)
              return group.name;
          }
          else
          {
              User user = (User)Enumerable.FirstOrDefault<User>(this._knownUsers, (Func<User, bool>)(u => u.id == this._item.owner_id));
            if (user != null)
              return user.Name;
          }
        }
    ...
    public Visibility NoVideosVisibility
    {
      get
      {
        if (!this.IsVideo && this._item.count <= 0)
          return Visibility.Visible;
        return Visibility.Collapsed;
      }
    }
    ...
    public void HandleTap()
    {
      ...
      else
      {
        ...
        Navigator.Current.NavigateToVideo(false, Math.Abs(ownerId), ownerId < 0, false);
      }
    }
  }
}
