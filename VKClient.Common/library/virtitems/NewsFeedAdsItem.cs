  ...
  public class NewsFeedAdsItem : VirtualizableItemBase, ISupportChildHeightChange, IHaveUniqueKey, IHaveNewsfeedItemId
  {
    ...
    private void GenerateLayout()
    {
      if (this.NewsItem.ads == null || this.NewsItem.ads.Count <= 0 || !(this.NewsItem.ads[0].type == "post"))
        return;
      NewsItemDataWithUsersAndGroupsInfo wallPostWithInfo = new NewsItemDataWithUsersAndGroupsInfo() { WallPost = this.NewsItem.ads[0].post, Groups = this._knownGroups, Profiles = this._knownUsers };
      if (this.WallPostItem == null)
      {
        this.WallPostItem = new WallPostItem(this.Width, new Thickness(0.0, 66.0, 0.0, 0.0), true, wallPostWithInfo,  null, false,  null, false, false, true, true, this,  null);
        this.VirtualizableChildren.Add((IVirtualizable) this.WallPostItem);
      }
      if (this._headerItem != null)
        return;
      this._headerItem = new UCItem(this.Width, new Thickness(), (Func<UserControlVirtualizable>)(() =>
      {
        AdHeaderUC adHeaderUc = new AdHeaderUC();
        adHeaderUc.textBlock1.Text = (this.NewsItem.ads_title.Capitalize());
        adHeaderUc.textBlock2.Text = (this.NewsItem.ads[0].age_restriction);
        return (UserControlVirtualizable) adHeaderUc;
      }), (Func<double>) (() => 66.0),  null, 0.0, false);
    ...
    private void PostPixelEvent(string eventName)
    {
      ...
      if (!flag)
        return;
      EventAggregator.Current.Publish(adPixelEvent);
    }
    ...
    protected override void NotifyAboutImpression()
    {
      ...
      if (this.NewsItem.ads == null || this.NewsItem.ads.Count <= 0 || !(this.NewsItem.ads[0].type == "post"))
        return;
      EventAggregator.Current.Publish(new AdImpressionEvent()
      {
      ...
      EventAggregator.Current.Publish(viewPostEvent);
    }
    ...
    public string GetKey()
    {
      ...
      if ((wallPostItem != null ? wallPostItem.WallPost :  null) != null)
        str += string.Format("{0}_{1}", this.WallPostItem.WallPost.owner_id, this.WallPostItem.WallPost.id);
      return str;
    }
  }
}
