...
namespace VKClient.Common.Library
{
  public class GenericCollectionViewModel2<B, T> : ViewModelBase, IMarker, ISupportCollectionEdit, ISupportLoadMore, ISupportReload where B : class where T : class
  {
    ...
    private int GetCollectionCount()
    {
      ...
      {
        IItemsCount itemsCount = obj as IItemsCount;
        if (itemsCount != null)
    ...
    private void InitializeGroupedCollection()
    {
      ...
      Group<T> localGroup = this._localGroup;
      observableCollection.Add(localGroup);
      Group<T> globalGroup = this._globalGroup;
      observableCollection.Add(globalGroup);
    ...
    public void Reload()
    {
      this.LoadData(this._refresh, false, false, false,  null,  null, false);
    }
    ...
    public void LoadMoreIfNeeded(object linkedItem)
    {
      ...
      if ((!(linkedItem is T) || count >= this.OffsetKnob) && (count < this.OffsetKnob || list[count - this.OffsetKnob] != linkedItem) || this._isLoading)
        return;
      this.LoadData(false, false, false, false,  null,  null, false);
    }
    ...
    public bool ReadData(ListWithCount<T> listWithCount)
    {
      ...
      listWithCount.List.ForEach((Action<T>) (listItem =>
      {
        ...
        if (list1 != null)
        {
          ...
          if (list2 == null)
          {
            if (list1.Count <= 0)
              return;
            bool flag2 = false;
            IEnumerator enumerator = ((IEnumerable) list1).GetEnumerator();
            try
            {
              while (enumerator.MoveNext())
              {
                object current = enumerator.Current;
                IHaveUniqueKey haveUniqueKey = current as IHaveUniqueKey;
                if (haveUniqueKey != null && !this._collectionItemsDict.ContainsKey(haveUniqueKey.GetKey()))
                {
                  this.AddRemoveKeyToDict(current, true);
                  flag2 = true;
                }
              }
            }
          ...
          {
            IEnumerator enumerator = ((IEnumerable) list2).GetEnumerator();
            try
            {
              while (enumerator.MoveNext())
              {
        ...
        {
          bool flag2 = true;
          IHaveUniqueKey haveUniqueKey = listItem as IHaveUniqueKey;
          if (haveUniqueKey != null && this._collectionItemsDict.ContainsKey(haveUniqueKey.GetKey()))
            flag2 = false;
          if (!flag2)
            return;
          if (this.IsGroupedMode)
          {
            ...
            collection.Add(listItem);
          this.AddRemoveKeyToDict(listItem, true);
          receivedNewData = true;
        }
      }));
    ...
    private void ReadLocalData(List<T> data, Action callback)
    {
      Execute.ExecuteOnUIThread((Action) (() =>
      {
        ...
        {
          IHaveUniqueKey haveUniqueKey = obj as IHaveUniqueKey;
          if (haveUniqueKey == null || !this._collectionItemsDict.ContainsKey(haveUniqueKey.GetKey()))
          {
            this._localGroup.Add(obj);
            this.AddRemoveKeyToDict(obj, true);
    ...
    public void LoadMore()
    {
      if (this._status.ResultCode.HasValue)
        return;
      this.LoadData(false, true, false, false,  null,  null, false);
    }
