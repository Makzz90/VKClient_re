...
namespace VKClient.Common.Library.Games
{
  public class GameLeaderboardItemHeader
  {
    ...
    public string PointsStr
    {
      get
      {
        ...
        if (this._leaderboardItem == null || !int.TryParse(this._leaderboardItem.points, out result))
          return "";
        return string.Format("{0} {1}", result, UIStringFormatterHelper.FormatNumberOfSomething(result, CommonResources.Games_OnePoint, CommonResources.Games_TwoFourPoints, CommonResources.Games_FivePoints, true,  null, false));
      }
    }
    ...
    public Visibility OwnVisibility
    {
      get
      {
        if (this._user == null || this._user.id != AppGlobalStateManager.Current.LoggedInUserId)
          return Visibility.Collapsed;
        return Visibility.Visible;
      }
    }
