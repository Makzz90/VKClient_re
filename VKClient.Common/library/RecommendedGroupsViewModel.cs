...
namespace VKClient.Common.Library
{
  public class RecommendedGroupsViewModel : ViewModelBase, ICollectionDataProvider<VKList<Group>, SubscriptionItemHeader>, ICollectionDataProvider<GroupCatalogInfoExtended, CatalogCategoryHeader>
  {
    ...
    public Func<VKList<Group>, ListWithCount<SubscriptionItemHeader>> ConverterFunc
    {
      get
      {
          return (Func<VKList<Group>, ListWithCount<SubscriptionItemHeader>>)(vkList => new ListWithCount<SubscriptionItemHeader>() { TotalCount = vkList.items.Count, List = (List<SubscriptionItemHeader>)Enumerable.ToList<SubscriptionItemHeader>(Enumerable.Select<Group, SubscriptionItemHeader>(vkList.items, (Func<Group, SubscriptionItemHeader>)(g => new SubscriptionItemHeader(g)))) });
      }
    }

    Func<GroupCatalogInfoExtended, ListWithCount<CatalogCategoryHeader>> ICollectionDataProvider<GroupCatalogInfoExtended, CatalogCategoryHeader>.ConverterFunc//todo: right?
    {
      get
      {
        return (Func<GroupCatalogInfoExtended, ListWithCount<CatalogCategoryHeader>>) (gcie =>
        {
          ...
          if (!((IList) gcie.categories).IsNullOrEmpty())
          {
            listWithCount.TotalCount = gcie.categories.Count - 1;
            listWithCount.List = (List<CatalogCategoryHeader>)Enumerable.ToList<CatalogCategoryHeader>(Enumerable.Select<GroupCatalogCategoryPreview, CatalogCategoryHeader>(Enumerable.Where<GroupCatalogCategoryPreview>(gcie.categories, (Func<GroupCatalogCategoryPreview, bool>)(c => (uint)c.id > 0U)), (Func<GroupCatalogCategoryPreview, CatalogCategoryHeader>)(gccp => new CatalogCategoryHeader(gccp))));
          }
    ...
    {
      if (this._categoryId == 0)
        this.CatalogCategories.LoadData(false, false,  null, false);
