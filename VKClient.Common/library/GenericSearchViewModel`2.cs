...
{
  public class GenericSearchViewModel<B, T> : GenericSearchViewModelBase, IHandle<UserIsLoggedOutEvent>, IHandle, ICollectionDataProvider2<VKList<B>, T>, ILocalCollectionDataProvider<T> where B : class where T : class, ISearchableItemHeader<B>
  {
    ...
    public override string SearchString
    {
      ...
      set
      {
        ...
        this.SearchVM.LoadData(true, false, true, false,  null,  null, false);
      }
    }
    ...
    {
      if (this._searchDataProvider.LocalItems == null || !Enumerable.Any<T>(this._searchDataProvider.LocalItems))
      {
        callback(new List<T>());
      }
      else
      {
        string searchString1 = this._searchString;
        string searchString = (searchString1 != null ? searchString1.ToLowerInvariant().Trim() :  null) ?? "";
        if (string.IsNullOrWhiteSpace(searchString))
          callback(new List<T>());
        else
          callback(new List<T>((IEnumerable<T>) Enumerable.Where<T>(this._searchDataProvider.LocalItems, (Func<T, bool>) (item => item.Matches(searchString)))));
      }
    }
    ...
    {
      this.SearchVM.LoadData(refresh, suppressLoadingMessage, suppressSystemTrayProgress, clearCollectionOnRefresh,  null,  null, instantLoad);
    }
