...
namespace VKClient.Common.Library
{
  public class SearchHintHeader : IHaveUniqueKey, ISearchableItemHeader<SearchHint>
  {
    ...
    {
      if (string.IsNullOrEmpty(this._searchHint.description))
    ...
    private Visibility GetVerifiedVisiblity()
    {
      if (this.IsCommunityType)
      {
        if (this._searchHint.group.verified != 1)
          return Visibility.Collapsed;
        return Visibility.Visible;
      }
      if (!this.IsUserType)
    ...
    private Visibility GetOnlineVisiblity()
    {
      if (!this.IsUserType)
        return Visibility.Collapsed;
      if (this._searchHint.profile.online != 1 || this._searchHint.profile.online_mobile != 0)
        return Visibility.Collapsed;
      return Visibility.Visible;
    }
    ...
    {
      if (this._searchHint == null || !this.IsUserType)
        return Visibility.Collapsed;
      if (this._searchHint.profile.online_mobile != 1)
        return Visibility.Collapsed;
      return Visibility.Visible;
    }
    ...
    private string UpdateKey()
    {
      if (this._searchHint.group != null)
        return "group_" + this._searchHint.group.id;
      if (this._searchHint.profile != null)
        return "user_" + this._searchHint.profile.id;
    ...
    public bool Matches(IList<string> searchStrings)
    {
      ...
      try
      {
        while (((IEnumerator) enumerator).MoveNext())
        {
          ...
          flag = !string.IsNullOrEmpty(str2) && str2.StartsWith(current, (StringComparison) 3) || !string.IsNullOrEmpty(str1) && str1.StartsWith(current, (StringComparison) 3);
          if (!flag)
            break;
        }
      }
