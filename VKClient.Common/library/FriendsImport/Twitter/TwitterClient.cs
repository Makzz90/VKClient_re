  ...
  public class TwitterClient
  {
    ...
    public async Task<TwitterGenericResponse<List<TwitterUser>>> GetUsers(string usersType)
    {
      ...
      do
      {
        try
        {
          string requestUrl = string.Format("https://api.twitter.com/1.1/{0}/list.json?count=200&skip_status=true&include_user_entities=false", usersType);
          ...
          {
            usersResponse.Error = (TwitterError) Enumerable.First<TwitterError>(twitterErrorResponse.errors);
            break;
          }
    ...
    {
      return await this.PerformPostRequest("https://api.twitter.com/1.1/direct_messages/new.json", new List<KeyValuePair<string, string>>() { new KeyValuePair<string, string>("screen_name", screenName), new KeyValuePair<string, string>("text", text) });
    }
