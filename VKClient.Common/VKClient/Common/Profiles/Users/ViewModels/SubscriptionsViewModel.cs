...
namespace VKClient.Common.Profiles.Users.ViewModels
{
  public class SubscriptionsViewModel : ViewModelBase, ICollectionDataProvider<UsersAndGroups, UserGroupHeader>, ICollectionDataProvider<List<Group>, GroupHeader>
  {
    ...
    public GenericCollectionViewModel<UsersAndGroups, UserGroupHeader> PagesVM { get; set; }

    public GenericCollectionViewModel<List<Group>, GroupHeader> GroupsVM { get; set; }
      ...
      get
      {
        return (Func<UsersAndGroups, ListWithCount<UserGroupHeader>>) (data =>
        {
          ...
          if (data.pages != null)
          {
            ...
            listWithCount.List.AddRange(data.pages.Select<Group, UserGroupHeader>((Func<Group, UserGroupHeader>) (page => new UserGroupHeader()
            {
              GroupHeader = new GroupHeader(page, (User) null)
            })));
          }
          if (data.users != null)
          {
            listWithCount.TotalCount += data.users.Count;
            listWithCount.List.AddRange(data.users.Select<User, UserGroupHeader>((Func<User, UserGroupHeader>) (user => new UserGroupHeader()
            {
      ...
      get
      {
        return (Func<List<Group>, ListWithCount<GroupHeader>>) (data =>
        {
          ...
          if (data != null)
          {
            ...
            listWithCount.List.AddRange(data.Select<Group, GroupHeader>((Func<Group, GroupHeader>) (group => new GroupHeader(group, (User) null))));
          }
          return listWithCount;
        });
      }
    ...
    public void LoadData(bool refresh = false, Action<bool> callback = null)
    {
      ...
      this.GroupsVM.LoadData(refresh, false, (Action<BackendResult<List<Group>, ResultCode>>) null, false);
    }
