    ...
    public class PushChannelManager
    {
        ...
        public void OpenChannel()
        {
            ...
            if (notificationChannel1 != null && notificationChannel1.ChannelUri == null)
            {
                ...
                notificationChannel1 = (HttpNotificationChannel)null;
            }
            if (notificationChannel1 == null)
            {
                try
                {
                    ...
                    notificationChannel2.ChannelUriUpdated += this.PushChannel_ChannelUriUpdated;
                    notificationChannel2.ErrorOccurred += this.PushChannel_ErrorOccurred;
                    notificationChannel2.ShellToastNotificationReceived += this.pushChannel_ShellToastNotificationReceived;
        ...
        private void pushChannel_ShellToastNotificationReceived(object sender, NotificationEventArgs e)
        {
            ...
            Execute.ExecuteOnUIThread((Action)(() =>
            {
                ...
                if (parameters.ContainsKey("device_token") && parameters.ContainsKey("url"))
                    tapCallback = (Action)(() => Navigator.Current.NavigateToWebViewPage(parameters["url"]));
                    ...
                    tapCallback = (Action)(() => VKRequestsDispatcher.DispatchRequestToVK<object>("account.validateAction", new Dictionary<string, string>()
                      {
                        {
                          "confirm",
                          MessageBox.Show(args["wp:Text2"], CommonResources.VK, MessageBoxButton.OKCancel) == MessageBoxResult.OK ? "1" : "0"
                        },
                        {
                          "hash",
                          parameters["confirm_hash"]
                        }
                      }, (Action<BackendResult<object, ResultCode>>)(f => { }), null, false, true, new CancellationToken?()));
                            if (tapCallback == null)
                                return;
        ...
        {
            EventAggregator.Current.Publish((object)new ChannelUriUpdatedEvent()
            {
                ChannelUri = uri
            });
        }
        ...
        {
            Logger.Instance.Error(string.Format("A push notification {0} error occurred.  {1} ({2}) {3}", (object)e.ErrorType, (object)e.Message, (object)e.ErrorCode, (object)e.ErrorAdditionalData));
