...
namespace VKClient.Common.Library
{
  public class PollVotersViewModel : ViewModelBase, ICollectionDataProvider<UsersListWithCount, FriendHeader>
  {
    ...
    public Func<UsersListWithCount, ListWithCount<FriendHeader>> ConverterFunc
    {
      get
      {
        return (Func<UsersListWithCount, ListWithCount<FriendHeader>>) (usersList => new ListWithCount<FriendHeader>()
        {
          TotalCount = usersList.count,
          List = new List<FriendHeader>(usersList.users.Select<User, FriendHeader>((Func<User, FriendHeader>) (u => new FriendHeader(u, false))))
        });
      }
    }
    ...
    {
      this._votersVM.LoadData(false, false, (Action<BackendResult<UsersListWithCount, ResultCode>>) null, false);
    }
