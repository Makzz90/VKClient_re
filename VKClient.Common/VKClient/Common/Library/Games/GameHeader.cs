...
namespace VKClient.Common.Library.Games
{
  public class GameHeader : ViewModelBase, IHandle<GameDisconnectedEvent>, IHandle
  {
    ...
    public Visibility IsNewVisibility
    {
      get
      {
        return this._game.is_new != 1 ? Visibility.Collapsed : Visibility.Visible;
      }
    }
    ...
    public long LastRequestDate
    {
      get
      {
        if (!this.Requests.IsNullOrEmpty())
          return this.Requests.OrderByDescending<GameRequestHeader, long>((Func<GameRequestHeader, long>) (req => req.GameRequest.date)).Last<GameRequestHeader>().GameRequest.date;
        return 0;
      }
    }
