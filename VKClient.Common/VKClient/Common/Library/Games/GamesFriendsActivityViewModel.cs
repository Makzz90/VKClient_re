    ...
    public Func<GamesFriendsActivityResponse, ListWithCount<GameActivityHeader>> ConverterFunc
    {
      get
      {
        return (Func<GamesFriendsActivityResponse, ListWithCount<GameActivityHeader>>) (data =>
        {
          ...
          ListWithCount<GameActivityHeader> listWithCount = new ListWithCount<GameActivityHeader>()
          {
            TotalCount = data.count
          };
          foreach (GameActivity gameActivity1 in data.items)
          {
            GameActivity gameActivity = gameActivity1;
            Game game = data.apps.FirstOrDefault<Game>((Func<Game, bool>) (app => app.id == gameActivity.app_id));
            User user = data.profiles.FirstOrDefault<User>((Func<User, bool>) (p => p.id == gameActivity.user_id));
