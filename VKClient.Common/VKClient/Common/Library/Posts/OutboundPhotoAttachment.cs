    ...
    public class OutboundPhotoAttachment : OutboundAttachmentBase
    {
        ...
        public override OutboundAttachmentUploadState UploadState
        {
            ...
            set
            {
                ...
                this.NotifyPropertyChanged<double>((System.Linq.Expressions.Expression<Func<double>>)(() => this.ImageOpacity));
        ...
        public Visibility IsUploadingVisibility
        {
            get
            {
                return this.UploadState != OutboundAttachmentUploadState.Uploading ? Visibility.Collapsed : Visibility.Visible;
            }
        }
        ...
        public override Attachment GetAttachment()
        {
            if (this._photo == null)
                return new Attachment()
                {
                    ...
                    photo = new Photo()
                    {
                        src_big = this.LocalUrl,
                        date = Extensions.DateTimeToUnixTimestamp(DateTime.Now.ToUniversalTime(), true),
                        owner_id = AppGlobalStateManager.Current.LoggedInUserId,
                        user_id = AppGlobalStateManager.Current.LoggedInUserId,
                        album_id = -3,
                        can_comment = 0,
                        likes = new Likes()
                        {
        ...
        public static OutboundPhotoAttachment CreateForUploadNewPhoto(Stream stream, long userOrGroupId = 0, bool isGroup = false, Stream previewStream = null, PostType postType = PostType.WallPost)
        {
            ...
            outboundPhotoAttachment._localUri = "/" + guid.ToString();
        ...
        {
            string str = string.Format("photo{0}_{1}", (object)ownerId, (object)id);
            ...
            Execute.ExecuteOnUIThread((Action)(() =>
            {
                ...
                else
                {
                    ...
                    ImagePreprocessor.PreprocessImage(stream, VKConstants.ResizedImageSize, true, (Action<ImagePreprocessResult>)(pres =>
                    {
                        ...
                        stream2.Close();
                        this.UploadImpl(bytes, /*action ?? (action =*/new Action(() =>
                        {
        ...
        private void UploadToImpl(Action<byte[], Action<BackendResult<Photo, ResultCode>>, Action<double>, Cancellation> uploadAction, byte[] bytes, Action completionCallback, Action<double> progressCallback)
        {
            ...
            uploadAction(bytes, (Action<BackendResult<Photo, ResultCode>>)(res =>
            {
                ...
                {
                    Logger.Instance.Info("!!!!!!!!!!!!FAILED TO UPLOAD");
                    this.UploadState = OutboundAttachmentUploadState.Failed;
                    if (this._retryFlag)
                    {
                        this._retryFlag = false;
                        this.Upload(completionCallback, (Action<double>)null);
                    }
                    else
                        completionCallback();
                }
