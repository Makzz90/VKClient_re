...
namespace VKClient.Common.BLExtensions
{
  public static class WallPostExtensions
  {
    ...
    {
      return wallPost != null && wallPost.from_id != AppGlobalStateManager.Current.LoggedInUserId;
    }
    ...
    public static void NavigateToPublishWallPost(this WallPost wallPost, int adminLevel)
    {
      if (wallPost == null)
        return;
      ParametersRepository.SetParameterForId("PublishWallPost", (object) wallPost);
      Navigator.Current.NavigateToNewWallPost(Math.Abs(wallPost.to_id), wallPost.to_id < 0L, adminLevel, false, false, false);
    }
    ...
    public static bool AskConfirmationAndDelete(this WallPost wallPost)
    {
      ...
      EventAggregator.Current.Publish((object) new WallPostDeleted()
      {
        WallPost = wallPost
      });
