...
namespace VKClient.Common.Framework
{
    public static class BrowserNavigationService
    {
        ...
        public static readonly DependencyProperty DisableHyperlinksProperty = DependencyProperty.RegisterAttached("DisableHyperlinks", typeof(bool), typeof(BrowserNavigationService), new PropertyMetadata((object)false));
        public static readonly DependencyProperty TextIdProperty = DependencyProperty.RegisterAttached("TextId", typeof(string), typeof(BrowserNavigationService), new PropertyMetadata((object)""));
        private static List<string> _flagsPrefixes = new List<string>()
    {
      "D83CDDE6",
      "D83CDDE7",
      "D83CDDE8",
      ...
      "D83CDDFF"
        ...
        private static List<string> _modificatableSmiles = new List<string>()
    {
      "261D",
      "270A",
      "270B",
      ...
      "D83EDD18"
        ...
        private static List<string> _smilesModificators = new List<string>()
    {
      "D83CDFFB",
      "D83CDFFC",
      "D83CDFFD",
      ...
      "D83CDFFE"
        ...
        private static List<string> _domainsList = new List<string>()
    {
      "AC",
      "AD",
      "AE",
      ...
      "СРБ"
        ...
        private static Regex _regex_Mention = new Regex("\\[(id|club)(\\d+)(?:\\:([a-z0-9_\\-]+))?\\|([^\\$]+?)\\]", RegexOptions.IgnoreCase);
        ...
        public static Regex Regex_Tag { get { return new Regex("(^|[\\s.,:'\";>\\)\\(]?)(" + (object)BrowserNavigationService._regex_MatchedTag + ")(@" + (object)BrowserNavigationService._regex_Domain + ")?(?=$|[\\s\\.,:'\"&;?<\\)\\(]?)", RegexOptions.IgnoreCase); } }
        ...
        {
            d.SetValue(BrowserNavigationService.TextProperty, (object)value);
        }
        ...
        {
            d.SetValue(BrowserNavigationService.DisableHyperlinksProperty, (object)value);
        }
        ...
        {
            d.SetValue(BrowserNavigationService.TextIdProperty, (object)value);
        }

        public static Run GetRunWithStyle(string text, RichTextBox richTextBox)
        {
            Run run = new Run();
            FontFamily fontFamily = richTextBox.FontFamily;
            run.FontFamily = fontFamily;
            double fontSize = richTextBox.FontSize;
            run.FontSize = fontSize;
            Brush foreground = richTextBox.Foreground;
        ...
        private static void AddRawText(RichTextBox text_block, Paragraph par, string raw_text)
        {
                ...
                else
                {
                    ...
                    if (name != null)
                    {
                        ...
                        if (@string != string.Empty)
                            par.Inlines.Add((Inline)BrowserNavigationService.GetRunWithStyle(@string, text_block));
                        par.Inlines.Add((Inline)BrowserNavigationService.GetImage(name));
                    }
                    else
            ...
            if (!(string1 != string.Empty))
                return;
            par.Inlines.Add((Inline)BrowserNavigationService.GetRunWithStyle(string1, text_block));
        }
        ...
        private static InlineUIContainer GetImage(string name)
        {
            ...
            image.Source = (ImageSource)new BitmapImage(new Uri(string.Format("/Resources/Emoji/{0}.png", (object)name), UriKind.RelativeOrAbsolute));
            image.Height = 25.0;
            image.Width = 25.0;
            image.Margin = new Thickness(0.0, 0.0, 4.0, -5.0);
            return new InlineUIContainer()
            {
                Child = (UIElement)image
            };
        }
        ...
        public static string CutTextGently(string text, int preferredMaxLength)
        {
            ...
            MatchCollection matchCollection3 = BrowserNavigationService._regex_Mention.Matches(text);
            MatchCollection matchCollection4 = BrowserNavigationService.Regex_Tag.Matches(text);
        ...
        public static List<string> ParseText(string text)
        {
            ...
            text = BrowserNavigationService._regex_Uri.Replace(text, (MatchEvaluator)(m =>
            {
                ...
                return string.Format("\a{0}\b{1}\a", (object)(str2 + str3), (object)str4).Replace("#", "\0");
            }));
            text = text.ReplaceByRegex(BrowserNavigationService._regex_Email, "\amailto:$2\b$2\a").ReplaceByRegex(BrowserNavigationService._regex_Mention, "\ahttps://vk.com/$1$2$3\b$4\a").ReplaceByRegex(BrowserNavigationService.Regex_Tag, string.Format("$1\ahttps://{0}$3$4\b$3$4\a", (object)BrowserNavigationService._searchFeedPrefix)).Replace("\n ", "\n").Replace(" \n", "\n").Replace("\0", "#");
            if (text.StartsWith(" "))
                text = text.Remove(0, 1);
            return ((IEnumerable<string>)text.Split('\a')).ToList<string>();
        }
    }
}
