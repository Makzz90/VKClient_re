...
namespace VKClient.Common.Framework.SharpDXExt
{
  public class MediaPlayer : IDisposable
  {
    ...
    public void Initialize(SharpDXContext context)
    {
      lock (this.lockObject)
      {
        ...
        using (MediaEngineClassFactory factory = new MediaEngineClassFactory())
          this.mediaEngine = new MediaEngine(factory, this.attributes, MediaEngineCreateFlags.None, new MediaEngineNotifyDelegate(this.OnMediaEngineEvent));
    ...
    public void OnRender()
    {
      ...
      {
        long ptsRef;
        if (this.isVideoStopped || this.mediaEngineEx == null || (this.sharpDxContext.BackBuffer == null || !this.mediaEngineEx.OnVideoStreamTick(out ptsRef)))
          return;
        Texture2D backBuffer = this.sharpDxContext.BackBuffer;
        if (backBuffer == null)
          return;
        Texture2DDescription description = backBuffer.Description;
        Rectangle dstRef = new Rectangle(0, 0, description.Width, description.Height);
        this.mediaEngineEx.TransferVideoFrame((ComObject) backBuffer, new VideoNormalizedRect?(), dstRef, new ColorBGRA?((ColorBGRA) this.BackgroundColor));
      }
    }
