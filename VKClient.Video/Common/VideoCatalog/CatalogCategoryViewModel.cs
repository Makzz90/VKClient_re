...
namespace VKClient.Common.VideoCatalog
{
    public class CatalogCategoryViewModel : ViewModelBase, IHandle<GroupMembershipStatusUpdated>, IHandle
    {
        ...
        private void HandleShowAllAction()
        {
            ...
            {
                if (id == "feed")
                    Navigator.Current.NavigateToNewsFeed(-100, false);
                else if (this._videoCategory.type == "channel")
                {
                    Navigator.Current.NavigateToVideo(false, Math.Abs(this.OwnerId), this.OwnerId < 0L, false);
                    flag = true;
                }
            }
            else
                Navigator.Current.NavigateToVideo(false, 0L, false, false);
            if (flag)
                return;
